...
class Network(nn.Module):
    ...
    def __init__(self, config, dataset_names):
        ...
        self.transformer_layer = nn.TransformerEncoderLayer(32, 1)
        self.transformer_enc = nn.TransformerEncoder(self.transformer_layer, num_layers = 2)

        #TODO: use full transformer (encoder+decoder)
        self.transformer = nn.Transformer(d_model=32, nhead=8, num_encoder_layers=12)

        self.spatial_fc = nn.Conv2d(10, 32, kernel_size=1)
        self.GCN = ContextBlock2(inplanes=32, ratio=1, pooling_type='att')


        if self.config.plane_refine_5:
            self.plane_refine = Plane_refine_block_5(32, config)
            self.fc3 = pt_utils.Conv2d(64+32, 32, kernel_size=(1, 1), bn=True)
    ...
    def forward(self, end_points, split):
        ...
        # ###########################Decoder############################
         ...
         # conbine
        plane_geo_features = end_points['plane_features'].permute(0,2,1).unsqueeze(-1) # B,c,m,1
        plane_geo_features = self.spatial_fc(plane_geo_features)
        features2 = self.fc1(features)  # 64
        features = self.fc2(features2)  # 32
        if self.config.plane_refine_5:

            plane_masks = end_points['plane_masks'] # B,m,40960
            result_list, plane_result = self.plane_refine(features, plane_masks)
            end_points['plane_results'] = plane_result

            # TODO: use attention module
            # plane_geo_features = self.GCN(plane_geo_features).permute(0,2,1,3).squeeze()# B,N,32

            # TODO: 1、use transformer encoder
            plane_geo_features = self.transformer_enc(plane_geo_features.squeeze().permute(0,2,1)) # B,N,32

            # TODO: 2、use full trainsformer
            # on_plane_features = []
            # for result in result_list:
            #     on_plane_features.append(torch.cat(result['on_plane_trans_feature_list']))
            # on_plane_features = torch.stack(on_plane_features)
            # plane_geo_features = self.transformer(on_plane_features.squeeze(), plane_geo_features.squeeze().permute(0,2,1)) # B,N,32

            plane_features = torch.ones([features.shape[0], 64, features.shape[2], features.shape[3]]).cuda()
            for k in range(len(result_list)):
                ...
                for l in range(len(result['on_plane_idx_list'])):
                    if len(result['on_plane_idx_list'][l]) != 0:
                        ...
                        plane_features[k, :, on_plane_idx, :] = torch.cat([on_plane_feature, plane_geo_features[k,l].view(1,32,1,1)], dim=1)
                    ...
                    if len(result['off_plane_idx_list'][l]) != 0:
                        ...
                        plane_features[k, :, off_plane_idx, :] = torch.cat([off_plane_feature, plane_geo_features[k,l].view(1,32,1,1)], dim=1)

            # TODO: 3、try multi loss( ori feature loss + plane refine feature loss )
            # ori_feature = features
...
class ContextBlock2(nn.Module):
    def __init__(self,inplanes,ratio,pooling_type='att',
                 fusion_types=('channel_add', ), alpha=1., betta=1.):
        ...
        self.betta = nn.Parameter(torch.tensor(betta), requires_grad=True)

        if pooling_type == 'att':
            self.conv_mask = nn.Conv2d(inplanes, 1, kernel_size=1)
            self.softmax = nn.Softmax(dim=2)
            self.conv_E = nn.Conv2d(inplanes, inplanes, kernel_size=1)
        else:
            self.avg_pool = nn.AdaptiveAvgPool2d(1)
        if'channel_add'in fusion_types:
            self.channel_add_conv = nn.Sequential(
    ...
    def spatial_pool(self, x):
        ...
        if self.pooling_type == 'att':
            ...
            # [N,C,H*W]
            C = F.softmax(self.conv_E(x), dim=1).view(batch, channel, height * width).permute(0,2,1)
            E = torch.matmul(C, context.squeeze(-1).permute(0,2,1))
    ...
    def forward(self, x):
        ...
        if self.channel_add_conv is not None:
            # [N, C, 1, 1]
            channel_add_term = self.channel_add_conv(context)
            out = out + self.alpha * channel_add_term + self.betta * E.unsqueeze(1)
        return out
...
class Plane_refine_block_5(nn.Module):
    ...
    def forward(self, feature, plane_masks):
        # feature b,32,n,1 plane_masks b,m,n
        room_list =[]
        batch_size = plane_masks.shape[0]
        planes_length = plane_masks.shape[1]
        planes_points = plane_masks.shape[2]

        plane_feature = self.fc1(feature)
        plane_feature = self.fc2(plane_feature)  # b,32,n,1
        plane_result = self.fc3(plane_feature)  # b,1,n,1

        mask_1 = self.sigmoid(plane_result) > 0.5
        mask_2 = self.sigmoid(plane_result) <= 0.5
        final_mask1 = mask_1.repeat(1, planes_length, 1, 1) * plane_masks.unsqueeze(-1)  # b*m*n*1
        final_mask2 = mask_2.repeat(1, planes_length, 1, 1) * plane_masks.unsqueeze(-1)

        for i in range(batch_size):
            # every room
            room_result = self.get_nearby_plane_points(feature[i].unsqueeze(0), final_mask1[i], final_mask2[i])
            room_list.append(room_result)

        return room_list, plane_result.squeeze()

    def get_nearby_plane_points(self, feature, final_mask1, final_mask2):
        # feature 32,n,1  masks m,n  plane_geo_features m,1,12,1,1
        planes_length = final_mask1.shape[0]
        room_result = {}
        ...
        room_result['on_plane_trans_feature_list'] = []

        idx = torch.arange(40960)

        # plane_feature = plane_feature.view()
        for i in range(planes_length):
            # every plane
            mask_1 = final_mask1[i].squeeze() #every plane n*1
            mask_2 = final_mask2[i].squeeze()

            if len(torch.nonzero(mask_1)) != 0:
                idx_on_plane = idx[mask_1]
                room_result['on_plane_idx_list'].append(idx_on_plane)
                on_plane_feature = feature[:, :, mask_1]
                ...
                room_result['on_plane_trans_feature_list'].append(on_plane_feature)

            else:
                room_result['on_plane_idx_list'].append([])
                room_result['on_plane_feature_list'].append([])
                room_result['on_plane_trans_feature_list'].append(torch.zeros([1,32,1,1]).cuda())

            if len(torch.nonzero(mask_2)) != 0:
                idx_off_plane = idx[mask_2]
                room_result['off_plane_idx_list'].append(idx_off_plane)
                off_plane_feature = feature[:, :, mask_2]
...
class Att_pooling(nn.Module):
    ...
    def forward(self, feature_set, dim=3):
        ...
        att_scores = torch.where(torch.isnan(att_scores), torch.full_like(att_scores, 0), att_scores)
...
def compute_plane_loss(end_points):
    ...
    labels = end_points['labels']
    plane_results = end_points['plane_results'] # b,n
    plane_labels = end_points['plane_labels'] # b,m
    plane_masks = end_points['plane_masks'] # b,m,n
    # labels = labels.reshape(-1)
    ...
    for i in range(len(room_results)):
        # every room
        plane_result = plane_results[i] #n
        plane_mask = plane_masks[i] # m,n
        plane_label = plane_labels[i] #m

        for j in range(plane_masks.shape[1]):
            # every plane
            label = plane_label[j]
            if label == -1:
                continue
            mask = plane_mask[j]
            result_list.append(plane_result[mask])

            ori_label = labels[i, mask]
            # TODO: board and wall as one class
            # if label == 2:
            #     ori_label[ori_label == 11] = 2
            temp = torch.ones_like(ori_label).float()
            temp[ori_label != label] = 0.
            label_list.append(temp)
    # plane_result = torch.cat(result_list, dim=2)
    label_result = torch.cat(label_list, dim=0)
    loss = bce_loss(torch.cat(result_list, dim=0).squeeze(), label_result) * 10
...
def visual_plane_error(end_points):
    ...
    cls_labels = torch.ones_like(labels)

   # FIXME data unbalance
    sigmoid = torch.nn.Sigmoid()

    for i in range(plane_results.shape[0]):
        # every room
        # room_result = room_results[i]
        plane_result = sigmoid(plane_results[i]) # n
        plane_result[plane_result > 0.5] = 1.
        plane_result[plane_result <= 0.5] = 0.

        result = torch.ones_like(labels[i]) * -1 # 40960
        pred = torch.ones_like(labels[i]) * -1 #40960
        cls_label = torch.ones_like(labels[i]) * -1
        for j in range(plane_masks.shape[1]):
            # every plane
            plane_mask = plane_masks[i][j]
            plane_label = plane_labels[i][j]

            label = labels[i, plane_mask] #
            temp = torch.ones_like(label).float() # true_label
            temp[label != plane_label] = 0.
            cls_label[plane_mask] = temp.long()

            idx1 = torch.where(plane_result[plane_mask] == temp) # pred result diff
            idx2 = torch.where(plane_result[plane_mask] != temp)
            temp[idx1] = 1
            temp[idx2] = 0
            result[plane_mask] = temp.long()
            pred[plane_mask] = plane_result[plane_mask].long()
        labels[i] = result
        preds[i] = pred
        cls_labels[i] = cls_label

    end_points['plane_visual'] = labels
    end_points['plane_visual_pred'] = preds
    end_points['plane_visual_label'] = cls_labels
