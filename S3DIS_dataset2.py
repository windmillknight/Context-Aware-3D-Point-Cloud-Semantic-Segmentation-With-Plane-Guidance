...
class S3DIS(torch_data.Dataset):
    def __init__(self, test_area_idx):
        ...
        self.plane_path = '/data/wty/S3DIS/plane_10000/processed'
...
class ActiveLearningSampler(IterableDataset):
    ...
    def get_nearby_plane_points(self, clouds, pick_point ,planes):
        planes_length = len(planes)
        # room_result = {}
        label_list = []
        mask_list = []
        ori_plane_feature_list = []
        idx = torch.arange(len(clouds))


        for i in range(planes_length):
            plane_args = planes.iloc[i]
            center = torch.tensor(plane_args[1:4]).double()
            normal = torch.tensor(plane_args[4:7]).double()
            xyz_min = torch.tensor(plane_args[9:12]).double()
            xyz_max = torch.tensor(plane_args[12:15]).double()
            label = torch.tensor(plane_args[15])
            ...
            ori_plane_feature = torch.cat([center - torch.tensor(pick_point.squeeze()), normal, xyz_max - xyz_min]).float()
            ...
            if len(idx_temp) > 1:
                ...
                ori_plane_feature_list.append(ori_plane_feature)

        label_list = torch.stack(label_list)
        mask_list = torch.stack(mask_list)
        ori_plane_feature_list = torch.stack(ori_plane_feature_list)
        plane_relation = torch.mean(torch.matmul(ori_plane_feature_list[:,3:6] ,ori_plane_feature_list[:,3:6].T),dim=1).unsqueeze(1)
        ori_plane_feature_list = torch.cat([ori_plane_feature_list, plane_relation], dim=1)
        return label_list, mask_list, ori_plane_feature_list
    ...
    def spatially_regular_gen(self):
        ...
        for i in range(self.n_samples * self.batch_size):  # num_per_epoch
            ...
            # Collect plane mask
            plane_labels, plane_masks, plane_freatures = self.get_nearby_plane_points(torch.tensor(queried_pc_xyz + pick_point), pick_point,
                                                                                      planes)
            ...
            yield queried_pc_xyz, queried_pc_colors, queried_pc_labels, queried_idx, cloud_idx, queried_pick_point,\
                  plane_labels, plane_masks, plane_freatures
    ...
    def collate_fn(self, batch):
        ...
        selected_pc, selected_colors, selected_labels, selected_idx, cloud_ind, selected_center, plane_labels, plane_masks, plane_features \
            = [], [], [], [], [], [], [], [], []
        ...
        for i in range(len(batch)):
            ...
            plane_labels.append(batch[i][6])
            plane_masks.append(batch[i][7])
            plane_features.append(batch[i][8])
            pre.append(len(torch.nonzero(batch[i][7]))/40960)
        ...
        max_number = max([len(i) for i in plane_labels])
        for i in range(len(batch)):
            while len(plane_labels[i]) < max_number:
                plane_labels[i] = torch.cat([plane_labels[i],torch.tensor([-1])])
                plane_masks[i] = torch.cat([plane_masks[i], torch.zeros_like(plane_masks[i][0]).unsqueeze(0)])
                plane_features[i] = torch.cat([plane_features[i], torch.zeros_like(plane_features[i][0]).unsqueeze(0)])

        plane_labels = torch.stack(plane_labels)
        plane_masks = torch.stack(plane_masks)
        ...
        inputs['plane_labels'] = plane_labels
        inputs['plane_masks'] = plane_masks
        inputs['plane_features'] = plane_features
        return inputs


if __name__ == '__main__':
    dataset = S3DIS(5)
    datasample = ActiveLearningSampler(dataset, split='training')
    dataloader = DataLoader(datasample, batch_size=6, collate_fn=datasample.collate_fn)
    pre = []
    for batch_idx, batch_data in enumerate(dataloader):

        print(batch_data[1])
        pre.append(batch_data[1])
