...
class ConfigS3DIS:
    ...
    max_epoch = 100  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    learning_rate_plane_refine = 1e-3
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
    ...
    plane_refine = False
    plane_refine_2 = False
    plane_refine_3 = False
    plane_refine_4 = False
    plane_refine_5 = True
    plane_refine_6 = False
    plane_refine_7 = False
...
class DataProcessing:
    ...
    @staticmethod
    def F1_from_confusions(confusions):
        """
        Computes IoU from confusion matrices.
        :param confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion matrices should be described by
        the last axes. n_c = number of classes
        ...
        mF1 = np.sum(F1Score, axis=-1, keepdims=True) / (counts + 1e-6)
