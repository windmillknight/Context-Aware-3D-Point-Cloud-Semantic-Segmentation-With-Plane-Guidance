...
dataset_path = '/data/wty/S3DIS/Stanford3dDataset_v1.2_Aligned_Version'
...
sub_grid_size = 0.04
original_pc_folder = join(dirname(dataset_path), 'boundary_original_ply')
...
def convert_pc2ply(anno_path, save_path):
    ...
    # kd_tree
    cloud_tree = KDTree(xyz)
    # check neighborhood
    for i in tqdm(range(points_num)):
        # print(f'{i} + : +{points_num}')
        center_class = labels[i]
        # check radius neighbors (r = 0.02)
        inds = cloud_tree.query_radius(xyz[i].reshape(1, -1), 0.02)[0]
        # flag indicating if this point is boundary
        flag = 0

        for j in inds:
            # when class change is found
            if (labels[j] != center_class):
                # flag = 1
                # boundary_class_0[i] = center_class
                # if boundary_class_1[i] == -1:
                #     boundary_class_1[i] = labels[j]
                # elif boundary_class_1[i] == labels[j]:
                #     continue
                # else:
                #     boundary_class_2[i] = labels[j]
        # if flag:
                is_boundaries[i] = 1
                break


    write_ply(save_path, (xyz, colors, labels, is_boundaries),
              ['x', 'y', 'z', 'red', 'green', 'blue', 'class', 'is_boundary'])
    print(anno_path + 'finished')
...
if __name__ == '__main__':
    ...
    for annotation_path in anno_paths:
        ...
        # if os.path.exists(join(original_pc_folder, out_file_name)):
        #     continue
        # p.apply_async(convert_pc2ply, args=(annotation_path, join(original_pc_folder, out_file_name)))
